...
class Machine:
    ...
    def check_collision(self,playerXY):
        ...
        above_bottomLeft = playerXY[0] >= self.bottomLeft[0] and playerXY[1] >= self.bottomLeft[1]
        above_bottomRight = playerXY[0] >= self.bottomRight[0] and playerXY[1] <= self.bottomRight[1]
        below_upperLeft = playerXY[0] <= self.upperLeft[0] and playerXY[1] >= self.upperLeft[1]
        below_upperRight = playerXY[0] <= self.upperLeft[0] and playerXY[1] <= self.upperRight[1]
...
class Trial:
    ...
    def check_in_foyer(self,timestamp,foyer_line):
        ...
        current_position = next(iter(processed_data.values()))[0]
        # assuming x increases as you go deeper and y increases as you go right
    ...
    def get_specific_walk_pos(self,timestamp):
        return next(iter(self.walk.get(timestamp).values()))[0]
